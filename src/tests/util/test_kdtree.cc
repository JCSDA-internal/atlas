...
static const IndexKDTree& nanoflann_search() {
    static IndexKDTree kdtree = []() {
        auto index_ = std::make_shared<util::detail::KDTree_nanoflann<idx_t, Point3>>();
        util::IndexKDTree kdtree{index_, geometry()};
        auto grid = Grid{"O32"};
        kdtree.build(grid.lonlat(), PayloadGenerator(grid.size()));
        return kdtree;
    }();
...
CASE("compare eckit and nanoflann kdtrees - cs to random") {
    ...
    std::vector<idx_t> random_payloads;
    random_points.reserve(points.size());
    random_payloads.reserve(random_points.size());
    for (size_t i = 0; i < points.size(); ++i) {
        double lon = lon_dist(gen);
        double lat = lat_dist(gen);
        random_points.emplace_back(lon, lat);
    }
    

    // time nanoflann kd-tree.
    auto [nanoflann_build_duration, nanoflann_query_duration] = [&]() {
        auto index_ = std::make_shared<util::detail::KDTree_nanoflann<idx_t, Point3>>();
        util::IndexKDTree tree_{index_, geometry()};
        ...
        for (const auto& p : random_points) {
            auto neighbours = tree_.closestPoints(p, 8).payloads();
            random_payloads.emplace_back(neighbours.empty() ? -1 : neighbours[0]);
        }
        auto query_t1 = std::chrono::high_resolution_clock::now();
    
        auto build_duration = std::chrono::duration_cast<std::chrono::milliseconds>(build_t1 - build_t0).count();
        auto query_duration = std::chrono::duration_cast<std::chrono::milliseconds>(query_t1 - query_t0).count();
        
        return std::make_pair(build_duration, query_duration);
    }();

    // time eckit kd-tree.
    auto [eckit_build_duration, eckit_query_duration] = [&]() {
    ...
    << "- build time = " << nanoflann_build_duration << " ms (speed-up = " << (double(eckit_build_duration) / double(nanoflann_build_duration)) << ")\n"
    << "- query time = " << nanoflann_query_duration << " ms (speed-up = " << (double(eckit_query_duration) / double(nanoflann_query_duration)) << ")\n";
}
...
CASE("compare eckit and nanoflann kdtrees - cs to gauss") {
    ...
    std::vector<idx_t> gauss_payloads;
    gauss_points.reserve(gaussGrid.size());
    gauss_payloads.reserve(gauss_points.size());
    ...
    auto [nanoflann_build_duration, nanoflann_query_duration] = [&]() {
        ...
        auto query_t0 = std::chrono::high_resolution_clock::now();
        for (const auto& p : gauss_points) {
            auto neighbours = tree_.closestPoints(p, 8).payloads();
            gauss_payloads.emplace_back(neighbours.empty() ? -1 : neighbours[0]);
        }
        auto query_t1 = std::chrono::high_resolution_clock::now();
...
CASE("test nanoflann kdtree") {
    auto grid = Grid{"O32"};

    auto index_ = std::make_shared<util::detail::KDTree_nanoflann<idx_t, Point3>>();
    util::IndexKDTree search{index_, geometry()};
    EXPECT(search.empty());
...
CASE("test nanoflann closestPoint") {
...
CASE("test nanoflann closestPoints") {
    auto neighbours          = nanoflann_search().closestPoints(PointLonLat{180., 45.}, 4).payloads();
    auto expected_neighbours = std::vector<idx_t>{760, 842, 759, 761};
    EXPECT_EQ(neighbours, expected_neighbours);
}

CASE("test nanoflann closestPointsWithinRadius") {
