...
ApproxInverse::SparseMatrixStorage ApproxInverse::halo_exchange(const SparseMatrixStorage& interp_matrix) const {
    ...
    auto global_row_send_buffer   = PartRidxBuffer{mpi::comm().size()};
    auto global_col_send_buffer   = PartRidxBuffer{mpi::comm().size()};
    auto value_send_buffer = ValueBuffer{mpi::comm().size()};
    ...
    const auto make_part_ridx_to_lidx_map = [&](const Views& views) {
        ...
        for (idx_t lidx = 0; lidx < views.partition.size(); ++lidx) {
            ...
            part_ridx_to_lidx_map.insert({part_ridx, lidx});
        }
    ...
    for (std::size_t rank = 0; rank < mpi::comm().size(); ++rank) {
        ...
        for (std::size_t i = 0; i < row_sub_buffer.size(); ++i) {
            ...
            ATLAS_ASSERT_MSG(row_iter != target_part_ridx_to_lidx_map.end(),
                             "Cannot find a row for (partition, remote_index) = (" +
                                 std::to_string(row_part_ridx.first) + ", " + std::to_string(row_part_ridx.second) +
                                 ") on MPI rank " + std::to_string(mpi::rank()) +
                                 ". Try increase source functionspace halo size.");

            ATLAS_ASSERT_MSG(col_iter != source_part_ridx_to_lidx_map.end(),
                             "Cannot find a column for (partition, remote_index) = (" +
                                 std::to_string(col_part_ridx.first) + ", " + std::to_string(col_part_ridx.second) +
                                 ") on MPI rank " + std::to_string(mpi::rank()) +
                                 ". Try increase target functionspace halo size.");
