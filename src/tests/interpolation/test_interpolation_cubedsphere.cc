...
std::array<util::Config, 3> interpolationTypes = {
    util::Config("type", "finite-element") | util::Config("interpolation_polygon_type", "facet"),
    util::Config("type", "finite-element") | util::Config("interpolation_polygon_type", "spherical"),
...
CASE("cubedsphere_to_cubedsphere_interpolation") {
    ...
    for (util::Config config : interpolationTypes) {
        ...
        config.get("interpolation_polygon_type", polygonType);

        SECTION("using " + interpType + " " + polygonType) {
            ...
            if (polygonType != "") {
                polygonType = "_" + polygonType;
            }

            gmshOutput("cubedsphere_to_cubedsphere_source_" + interpType + polygonType + ".msh", FieldSet{sourceField});
            gmshOutput("cubedsphere_to_cubedsphere_target_" + interpType + polygonType + ".msh", FieldSet{targetField});
        }
    }
}
...
CASE("cubedsphere_scalar_interpolation") {
    ...
    for (util::Config config : interpolationTypes) {
        ...
        SECTION("using " + interpType + " " + polygonType) {
            ...
            gmshOutput("cubedsphere_source_" + interpType + polygonType + ".msh", FieldSet(sourceField));

            auto targetFields = FieldSet{};
            targetFields.add(targetField);
            targetFields.add(errorField);
            targetFields.add(partField);
            gmshOutput("cubedsphere_target_" + interpType + "_" + polygonType + ".msh", targetFields);
